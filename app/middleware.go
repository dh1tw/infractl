...
// redirected URL.
func (s *Server) apiRedirectRouter(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {

		if strings.Contains(req.URL.Path, "api") {
			res := s.apiMatch.Find([]byte(req.URL.String()))
			if len(res) == 0 {
				req.URL.Path = strings.Replace(req.URL.Path, "api", "api/v"+s.apiVersion+"", 1)
			}
		}
